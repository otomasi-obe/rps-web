...
public class JsonToDocxService {
    ...
    public JsonToDocxService() {
        // Use shared template from public/ directory
        Path projectDir = Path.of(System.getProperty("user.dir"));
        Path publicTemplate = projectDir.resolve("public/RPS.docx");
        Path fallback = Path.of("/home/ubuntu/rps-web/public/RPS.docx");

        if (Files.exists(publicTemplate)) {
            this.templatePath = publicTemplate.toString();
        } else if (Files.exists(fallback)) {
            this.templatePath = fallback.toString();
        } else {
